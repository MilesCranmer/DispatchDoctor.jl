...
- `include_closures::Bool=false`:
  - When set to `true`, the macro will also apply to closure functions defined within the stabilized code. By default, closure functions are skipped to maintain backward compatibility.
...
By default, this skips closure functions, but you can include them with `include_closures=true`.
...
To also apply to closure functions:

```julia
using DispatchDoctor: @stable

@stable include_closures=true function outer(x)
    inner() = x > 0 ? x : 0.0  # This closure will also be stabilized
    return inner()
end
```
